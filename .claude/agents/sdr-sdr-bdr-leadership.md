...
- **Preferred CTAs** (each = a deliverable give delivered ON a 15-min call — see `cta-offers.md`): **3 AI-drafted personalized
  emails** to their top 3 target accounts, **run-rate + signal-set estimate** (current meeting run
  rate + what the AI SDR adds on top), **signal play** (~4.4x), **signal-mapping session** (map
  signal sets, find the highest-yield sources), **outbound teardown** (3 fixes; best practices lift
  response 50–70%). Do NOT promise de-anonymized visitors or "25 in-market accounts" — we can't
  deliver those.
